...
class DefaultScreenLifecycleObserver : ScreenLifecycleObserver {
    ...
    override fun onActivityStateChanged(activityLifecycleState: ActivityLifecycleState) {
         ...
         * Example: we attach observer for the first time, events are Create -> Started -> Resumed.
         * Then re-attach, events are again Create -> Started -> Resumed (cux resume is the current state).
